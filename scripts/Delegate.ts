...
async function main() {
    ...
    if(voter.weight.toNumber() == 0) 
        throw new Error("You have no right to vote");

    if(voter.voted) 
        throw new Error("You already voted.");

    if(delegate_to[0] == signer.address) 
        throw new Error("Self-delegation is disallowed.");

    const delegate = await ballotContract.voters(delegate_to[0]);

    if(delegate.weight.toNumber() == 0) 
        throw new Error("Cannot delegate to accounts that cannot vote.");
    if(delegate.delegate == delegate_to[0]) 
