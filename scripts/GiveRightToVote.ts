...
async function main() {
    ...
    const ballotContract = ballotContractFactory.attach(contract);

    const chairperson = await ballotContract.chairperson();
    if(chairperson != signer.address) 
        throw new Error("Only chairperson can give right to vote.");
    
    const obj = await ballotContract.voters(voter_address[0]);
    if(obj.weight.toNumber() === 1) 
        throw new Error("can not give right to vote for someone that has already voting rights");
    if(obj.voted) 
        throw new Error("can not give right to vote for someone that has voted");
