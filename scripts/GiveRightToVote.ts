...
async function main() {
    ...
    try {
        ...
        const GiveRightToVoteTxn = await ballotContract.giveRightToVote(anotherSigner);
        const receipt = await GiveRightToVoteTxn.wait();
        console.log(`GiveRightToVote transaction completed successfully. Receipt: ${receipt}`);
    } catch(error) {
