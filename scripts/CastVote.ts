...
async function main() {
    ...
    await ballotContract.deployTransaction.wait(); // similar to ballotContract.deployed() in 
    
    try {
        const voteTxn = await ballotContract.vote(1);
        const receipt = await voteTxn.wait();
        console.log(`Vote transaction completed successfully. Receipt: ${receipt}`);
    } catch(error) {
